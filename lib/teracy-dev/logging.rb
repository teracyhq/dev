...
module TeracyDev
  module Logging
    ...
    def self.configure_logger_for(classname)
      ...
      logger.formatter = proc do |severity, datetime, progname, msg|
        ...
        @acceptors.each do |acceptor|
          unless acceptor.accept(severity, datetime, progname, msg)
            accepted = false
            break
          end
        end
        ...
        if accepted
          @filters.each do |flt|
            if flt.respond_to?(:filtered)
              deprecated_msg = '[TeracyDev::Logging][WARN]: filter.filtered is deprecated, ' \
              "use filter.filter instead for #{flt}"
          ...
          when 'DEBUG'
            ...
            # display the trace of the exact line numeber having
            # logger call (file, line number, function name)
            tracing = TeracyDev::Common.light_gray("#{caller(4..4).first}\n")
            log += tracing
          end
    ...
    private_class_method :configure_logger_for
  end
end
