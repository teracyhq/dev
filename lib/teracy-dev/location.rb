...
module TeracyDev
  class Location
    ...
    def self._check_ref(current_ref, ref_string)
    ...
    def self._check_tag(current_ref, desired_tag)
    ...
    def self._check_branch(current_ref, desired_branch)
      ...
      if File.exist? path
        if sync_existing == true
          ...
          Dir.chdir(path) do
            ...
            if ref
              self._check_ref(current_ref, ref)
            elsif tag
              self._check_tag(current_ref, tag)
            else
              branch ||= 'master'

              self._check_branch(current_ref, branch)
            end
          end
        end
      else
