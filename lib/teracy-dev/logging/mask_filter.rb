  ...
  module Logging
    ...
    class MaskFilter < Filter
      ...
      def resolve_variables(variables)
        ...
        if !override_variables.nil? && !override_variables.empty?
          # override variables existing keys with override_variables matching keys only
          variables.each do |key, _val|
            # puts "key: #{key}, val: #{val}, o_val: #{override_variables[key]}"
            variables[key] = override_variables[key] unless override_variables[key].nil?
          end
        end

        variables.each do |_key, val|
          val = val.to_s
      ...
      def add_values(values)
        ...
        # so that we can add value as a string or array of string
        @masked_values = @masked_values.to_a.flatten.to_set
      end
      ...
      def load_override_variables
        conf_override_path = File.join(TeracyDev::Env::EXTENSION_ENTRY_PATH, 'config_override.yaml')
        return nil unless File.exist? conf_override_path

        override_conf = Util.load_yaml_file(conf_override_path)
